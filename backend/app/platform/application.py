...
def create_platform(database_url=None, sandbox=None):
    ...
    async def lifespan(app):
        ...
        with Session(engine) as db, db.begin():
            seed_policy(db, sandbox)
            ensure_super_admin(db)
        yield
        engine.dispose()

    app = FastAPI(title="Ajo platform", version="0.3.0", lifespan=lifespan)
    app.state.ctx = ctx
    ...
    async def validation_error(request, exc):
        ...
        for error in exc.errors():
            ...
            kind = error.get("type", "")
            if field == "phone":
                message = "Enter your phone number with country code, for example +14165551234."
            elif field == "password":
        ...
        return JSONResponse(
            {"detail": "Please check the highlighted information.", "errors": errors},
            status_code=422,
        )
    ...
    app.include_router(auth.routes(ctx))
    app.include_router(circles.routes(ctx))
    app.include_router(operations.routes(ctx))
    app.include_router(cms.routes(ctx))
    ...
    def health():
        return {
            "status": "ok",
            "sandbox": sandbox,
            "live_payments": not sandbox and os.getenv("AJO_PAYMENT_PROVIDER", "unconfigured") != "unconfigured",
            "version": "0.3.0",
        }
    ...
    def ready():
        ...
        return {
            "database": "reachable",
            "canonical_database": "postgresql" if engine.dialect.name == "postgresql" else engine.dialect.name,
            "production_ready": False,
            "missing": ["approved live provider adapters", "approved jurisdiction and policies"],
        }

    static = Path(
        os.getenv(
            "AJO_FRONTEND_ASSETS",
            str(Path(__file__).resolve().parents[3] / "frontend" / "public" / "assets"),
        )
    )
