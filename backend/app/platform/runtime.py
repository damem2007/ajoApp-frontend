...
class Context:
    def __init__(self, engine, sandbox):
        ...
        self.vault = load_vault(sandbox)
        registry = ProviderRegistry(sandbox=sandbox)
        self.payments = registry.payments()
        self.identity = registry.identity()
        self.notifications = registry.notifications()
        self.dummy_password = password_hash("nonexistent-account-timing-placeholder")

        def session():
            if not self.vault:
                fail("Set AJO_KEY_FILE to a mounted encryption key; live providers remain unconfigured", 503)
            with Session(engine) as db:
                if engine.dialect.name == "sqlite":
                    db.execute(text("BEGIN IMMEDIATE"))
                else:
                    db.execute(text("SELECT pg_advisory_xact_lock(714206)"))
                try:
                    yield db
                    db.commit()
                except Exception:
    ...
    def staff(self, *roles):
        def check(user=Depends(self.actor)):
            if user.role not in roles:
                fail("Staff permission required", 403)
            if not self.sandbox and not user.mfa_enabled:
                fail("Staff MFA required", 403)
            return user
    ...
    def require_sandbox(self):
        if not self.sandbox:
            fail("Sandbox-only operation; live adapter is not configured", 503)
